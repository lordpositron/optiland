...
class BasinHopping(OptimizerGeneric):
    ...
    def optimize(self, niter=100, callback=None, minimizer_kwargs=None, **kwargs):
        ...
        Args:
            ...
            callback (callable): A function called after each iteration of basin-hopping.
                                 `callback(xk, f, accept)`
            minimizer_kwargs (dict, optional): Keyword arguments for the local minimizer (passed to
                                               `scipy.optimize.minimize`). For example,
                                               `minimizer_kwargs={'method':'L-BFGS-B', 'bounds': bounds}`.
                                               If bounds are provided here, they will be used by the
                                               local minimizer. Basin-hopping itself does not directly
                                               take a `bounds` argument for the global search.
            **kwargs: Additional keyword arguments to pass to `scipy.optimize.basinhopping`.
                      e.g., `T` (temperature), `stepsize`, `take_step`, `accept_test`, `interval`, `disp`.
        ...
        # Check if problem has bounds defined, and if so, warn if not handled by minimizer_kwargs
        problem_bounds = tuple([var.bounds for var in self.problem.variables])
        has_problem_bounds = not all(b[0] is None and b[1] is None for b in problem_bounds)
        ...
        if has_problem_bounds and 'bounds' not in minimizer_kwargs:
            print(
                ...
                "its default behavior or other settings in minimizer_kwargs handle them."
            )
