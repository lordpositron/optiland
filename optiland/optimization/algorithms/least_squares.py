...
class LeastSquares(OptimizerGeneric):
    ...
    def optimize(
        ...
        Args:
            maxiter (int, optional): Maximum number of function evaluations (NFEV).
                                      SciPy's least_squares uses max_nfev.
                                      If None, SciPy's default is used.
            ...
            method_choice (str, optional): Method for scipy.optimize.least_squares.
                                         ...
                                         Defaults to 'lm'.
        Returns:
            scipy.optimize.OptimizeResult: The optimization result object from SciPy.
        """
        ...
        if (
            ...
            for i in range(x0_numpy.shape[0]):
                ...
                if low_b != -be.inf and x0_numpy[i] <= low_b:
                    x0_numpy[i] = low_b + eps
                if upp_b != be.inf and x0_numpy[i] >= upp_b:
                    x0_numpy[i] = upp_b - eps
                # Clip if adjustment pushed it out of the other bound
                if x0_numpy[i] > upp_b and upp_b != be.inf:
                    x0_numpy[i] = upp_b
